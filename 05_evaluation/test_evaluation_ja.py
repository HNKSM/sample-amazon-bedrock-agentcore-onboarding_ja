"""
AWS Cost Estimatorエージェントの評価

このスクリプトは3つのモードで2つの次元（ツール使用 + 出力品質）を測定します:

  Local     (デフォルト)    - strands-agents-evalsでOutputEvaluator + ToolCallEvaluatorを使用
  On-Demand (--ondemand) - AgentCore Evaluation APIでBuiltin.Correctness
                           + ツール使用のカスタム評価器を使用
  Online    (--online)   - オンライン評価設定による継続的モニタリング
                           + AgentCore Runtime上でのエージェント呼び出し

使用方法:
    # ローカル評価（開発用）
    uv run python test_evaluation_ja.py

    # オンデマンド評価（エージェントはローカルで実行、AgentCore APIでスコアリング）
    uv run python test_evaluation_ja.py --ondemand

    # オンライン評価（Runtime上のエージェント、継続的モニタリング）
    uv run python test_evaluation_ja.py --online
"""
...
CASES: list[Case[str, str]] = [
    Case(
        name="single-ec2",
        input="us-east-1で24時間365日稼働するEC2 t3.microインスタンス1台",
    ...
    Case(
        name="multi-service",
        input="us-east-1でEC2 m5.largeインスタンス2台とRDS db.t3.micro",
        expected_output=None,
        expected_trajectory=["get_pricing"],
        metadata={"expected_tools": ["get_pricing", "execute_cost_calculation"]},
    ),
...
レスポンスが有用なAWSコスト見積もりを提供しているかを評価します。

基準:
- 具体的な金額（月額または時間単位のコスト）が含まれている
- 入力で言及されたAWSサービスがリストされている
- 合計またはサマリーのコスト数値が提供されている
- コストが要求されたサービスに対して妥当に見える

全ての基準を満たす場合は1.0をスコア。
一部の基準を満たすが見積もりが不完全な場合は0.5をスコア。
意味のあるコスト見積もりが提供されていない場合は0.0をスコア。
"""
...
# ---------------------------------------------------------------------------
# オンデマンド/オンラインツール使用評価用のカスタム評価器設定
# ---------------------------------------------------------------------------
# なぜBuiltin.ToolSelectionAccuracyではないのか？
#   そのビルトインはエージェントが*実行した*各ツール呼び出しを判定しますが、
#   エージェントがハルシネーションする場合（ツールを完全にスキップ）、
#   判定すべき呼び出しがゼロになるため、静かにパスしてしまいます。
#   価格ツール呼び出しの*不在*を検出できるTRACEレベルの評価器が必要です。
...
def get_or_create_evaluator() -> str:
    """AgentCore上でカスタム評価器を登録、または既存のものを再利用します。

    create_strands_evaluator()に渡す評価器IDを返します。
    """
