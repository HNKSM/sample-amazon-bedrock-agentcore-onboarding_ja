...
このスクリプトは、ゲートウェイ設定からの既存のCognito M2M OAuth設定と統合する
OAuth2認証情報プロバイダーを作成します。プロバイダーにより、AgentCore Identityは
認証されたAPI呼び出しのためにアクセストークンを安全に管理できます。
...
def setup_oauth2_credential_provider(provider_name: str = PROVIDER_NAME, force: bool = False) -> dict:
    ...
    2. Cognito検出URLを使用してOAuth2認証情報プロバイダーを作成
    ...
    if not has_cognito:
        ...
        # Gateway ClientからCognitoでOAuth認証器を作成するためのシンプルなインターフェースを使用
...
def wait_for_oidc_endpoint(oidc_url, max_wait=600, interval=30):
    """OIDC検出エンドポイントが利用可能になるまで待機
    
    実際のテストに基づくと、OIDCエンドポイントはDNS伝播とサービス初期化の遅延により、
    利用可能になるまで5分以上かかる場合があります。
    """
    ...
    while time.time() - start_time < max_wait:
        ...
        # HTTPエラーステータスコード（4xx、5xx）の例外を発生
