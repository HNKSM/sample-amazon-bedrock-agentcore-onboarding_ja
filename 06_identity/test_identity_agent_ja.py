"""
AgentCore Identityをcost_estimator_agent_with_identityを呼び出してテスト
...
def log_jwt_token_details(access_token: str) -> None:
    """
    Base64デコードを使用してデバッグ目的でJWTトークンの内容をログ出力
    ...
    Note:
        ...
        セキュリティ上の理由から、署名部分はデコードされません。
    """
    # デバッグのためにJWTトークンの部分を解析してログ出力
    token_parts = access_token.split(".")
    for i, part in enumerate(token_parts[:2]):  # ヘッダーとペイロードのみデコード、署名はしない
        try:
            # 必要に応じてパディングを追加（JWT Base64エンコーディングは末尾の'='文字を省略する場合がある）
...
# LLMに公開されるツール関数（access_tokenパラメータなし）
@tool(
    name="cost_estimator_tool",
    description="アーキテクチャの説明からAWSのコストを見積もります"
)
async def cost_estimator_tool(architecture_description: str) -> str:
    """
    アーキテクチャの説明に基づいてAWSコストを見積もります。
    ...
    """
    # 認証付きの内部関数を呼び出し
    # エージェントからアクセストークン引数を隠すために内部関数を呼び出します
